...
GedcomViewer.prototype = {
	...
	addBox : function(p, level) {
		...
		if (null != attrs.open.img) {
			p.box.open = this.r.image("/static/imgs/genealogy/" + attrs.open.img, x0 + W - imgW - 2, y0 + imgOffsetY,
		...
		if (attrs.tree.height > 0) {
			p.box.tree = this.r.image("/static/imgs/genealogy/" + attrs.tree.img, x0 + W - imgW - 2, y0 + imgOffsetY,
	...
	createHtmlLink : function(p, f) {
		...
		return '<a href="javascript:void(GedcomToolbox.show(\'' + (f ? f.id : p.id) + '\'));">'
				+ "<img src='/static/imgs/genealogy/" + GedcomToolbox.getSexeImage(p.getValue("sexe")) + "'/>" + " "
